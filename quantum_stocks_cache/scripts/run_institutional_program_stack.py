...
from quantum_trainer.institutional_program_stack import run_institutional_program_stack
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a local hedge-fund/quant program capability map."
    )
    parser.add_argument(
        "--stack-csv",
        default=str(PROJECT_ROOT / "configs" / "institutional_program_stack.seed.csv"),
        help="Input institutional program stack seed CSV.",
    )
    parser.add_argument(
        "--output-dir",
...
def main() -> int:
    try:
        ...
        output = run_institutional_program_stack(
            stack_csv=Path(args.stack_csv),
            output_dir=Path(args.output_dir),
        )
        summary = output.summary.iloc[0].to_dict()
        logger.info("Institutional program stack complete.")
        ...
        logger.exception("Institutional program stack failed: %s", exc)
        return 1
