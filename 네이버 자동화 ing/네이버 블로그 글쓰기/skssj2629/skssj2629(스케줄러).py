...
TASK_PREFIX = "NaverBlogAutoPost_2629"
...
PEER_SCHEDULE_FILES = [
    os.path.join(PARENT_DIR, "자동발행상태기록파일", "daily_schedule.json"),
    os.path.join(PARENT_DIR, "skssj2628", "자동발행상태기록파일", "daily_schedule.json"),
]
...
def print_schedule(items, target_date, schedule_path):
    ...
    print("=" * 60)
    for item in items:
        label = "일상" if item["post_type"] == "일상" else "네이버"
