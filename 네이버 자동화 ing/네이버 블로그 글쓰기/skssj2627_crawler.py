...
TARGET_CSV = Path(__file__).resolve().parent / "skssj2627_db.csv"
DEFAULT_USER_AGENT = (
...
TARGET_SUBCATEGORY_NAME = "계절가전"
TARGET_PRODUCT_NAME = "계절가전"
MIN_PRODUCT_PRICE = 0
...
class ProductRow:
    ...
    def to_csv_row(self) -> dict[str, object]:
        ...
        return {
            ...
            "수집경로": f"로켓배송>{TARGET_CATEGORY_NAME}>{TARGET_SUBCATEGORY_NAME}",
            "수집키워드": self.collection_keyword or TARGET_SUBCATEGORY_NAME,
        ...
        try:
            with used_json_path.open("r", encoding="utf-8") as f:
                ...
                for key, val in used_data.items():
                    ...
                    key_link = str(key or "").strip()
                    if name:
                        existing_names.add(name)
                    if link:
                        existing_urls.add(link)
                    if key_link:
                        existing_urls.add(key_link)
                    existing_product_keys.update(
                        build_product_identity_keys(
                            product_name=val.get("product_name", ""),
                            product_url=link or key_link,
                        )
                    )
        except Exception as exc:
...
def enrich_existing_csv_row(row: dict[str, str], fieldnames: list[str]) -> dict[str, str]:
    ...
    category_name = normalized.get(K_CATEGORY, "").strip() or TARGET_SUBCATEGORY_NAME
    ...
    fallback_values = {
        ...
        K_SOURCE_PATH: normalized.get(K_SOURCE_PATH, "").strip() or f"로켓배송>{TARGET_CATEGORY_NAME}>{category_name}",
        K_SOURCE_KEYWORD: normalized.get(K_SOURCE_KEYWORD, "").strip() or category_name,
    }

    for key, value in fallback_values.items():
        if key in normalized and not str(normalized.get(key, "")).strip() and value:
...
def click_category_flow(driver: webdriver.Chrome) -> None:
    ...
    click_category_by_name(driver, TARGET_SUBCATEGORY_NAME)
...
def extract_product_keyword(name: str) -> str:
    ...
    model_tokens = []
    product_markers = (
        "에어컨", "냉난방", "냉방", "난방", "제습기", "가습기", "선풍기", "서큘레이터",
        "냉풍기", "온풍기", "히터", "전기장판", "전기요", "온수매트", "공기청정기",
        "건조기", "보일러", "난로", "쿨러", "리모컨",
    )
    for token in tokens:
        if any(marker in token for marker in product_markers):
            product_type_tokens.append(token)
        if re.search(r"[A-Za-z]", token) and re.search(r"\d", token):
            model_tokens.append(token)

    selected = []
    for token in tokens[:4] + product_type_tokens[:2] + model_tokens[:2]:
        if token not in selected:
            selected.append(token)

    return " ".join(selected)[:60].strip() or name[:45]
...
def infer_metadata(product_name: str, keyword: str, category_name: str) -> dict[str, str]:
    base = {
        ...
        K_INTENT: "\uBB38\uC81C\uD574\uACB0\uD615 \uCD94\uCC9C",
        K_ANGLE: "\uAD6C\uB9E4 \uC804 \uD655\uC778\uD560 \uC870\uAC74\uC744 \uC815\uB9AC\uD558\uB294 \uCCB4\uD06C\uB9AC\uC2A4\uD2B8\uD615",
    ...
    rules = [
        ...
        (
            ("에어컨", "냉난방기", "이동식에어컨", "창문형"),
            {
                K_PRODUCT_GROUP: "에어컨/냉난방기",
                K_SCENARIO: "여름 전기요금과 설치 조건을 같이 보면서 냉방 제품을 골라야 할 때",
                K_TARGET: "원룸, 거실, 사무실처럼 공간 크기에 맞는 냉방 제품을 찾는 사람",
                K_PLACE: "원룸, 거실, 사무실, 작은 매장",
                K_SEASON: "여름, 장마철",
                K_PAIN: "평수, 설치 방식, 배관 조건을 놓치면 가격보다 설치에서 더 크게 막힐 수 있음",
                K_POINT1: "공간 면적과 설치 조건을 기준으로 판단하기 좋음",
                K_POINT2: "방문설치, 이동식, 창문형처럼 상황별 선택지가 분명함",
                K_POINT3: "계절 수요가 강해 구매 전 검색 의도가 뚜렷함",
                K_CAUTION: "설치비, 배관형, 실외기 위치, 전기요금 부담은 반드시 따로 확인해야 함",
            },
        ),
        (
            ("바람막이", "실링팬", "실외기", "에어컨 청소", "청소 물받이", "디플렉터"),
            {
                K_PRODUCT_GROUP: "에어컨 보조용품",
                K_SCENARIO: "에어컨은 있는데 바람 방향, 청소, 실외기 열 관리 때문에 불편할 때",
                K_TARGET: "기존 에어컨을 더 편하게 쓰고 싶은 원룸, 가정, 사무실 사용자",
                K_PLACE: "벽걸이 에어컨 주변, 천장형 에어컨 주변, 베란다, 실외기 주변",
                K_SEASON: "여름, 장마철",
                K_PAIN: "본체를 바꾸기에는 부담스럽지만 바람, 청소, 열기 문제는 바로 해결하고 싶은 상황",
                K_POINT1: "큰 가전 교체 없이 불편을 줄이는 보조용품 성격이 강함",
                K_POINT2: "가격 부담이 낮아 비교 후 바로 확인하기 쉬움",
                K_POINT3: "설치 방식과 호환 여부만 맞으면 만족도가 올라가기 쉬움",
                K_CAUTION: "에어컨 형태, 사이즈, 설치 가능 여부가 맞는지 먼저 확인해야 함",
            },
        ),
        (
            ("모기", "해충", "살충", "포충", "벌레"),
            {
                K_PRODUCT_GROUP: "해충퇴치용품",
                K_SCENARIO: "여름철 집 안에 모기나 날벌레가 늘어서 생활이 불편할 때",
                K_TARGET: "아이 있는 집, 원룸 거주자, 주방이나 베란다 벌레가 신경 쓰이는 사람",
                K_PLACE: "침실, 거실, 주방, 베란다",
                K_SEASON: "여름, 장마철",
                K_PAIN: "냄새, 소음, 안전성, 설치 위치를 모르고 고르면 오래 쓰기 어려움",
                K_POINT1: "생활 공간별로 필요한 방식이 달라 비교 포인트가 분명함",
                K_POINT2: "여름철 즉시성이 강해서 상세 조건 확인으로 이어지기 쉬움",
                K_POINT3: "소모품 여부와 관리 편의성을 함께 판단하기 좋음",
                K_CAUTION: "사용 공간, 아이나 반려동물 동선, 소모품 교체 여부를 확인해야 함",
            },
        ),
        (
            ("전기요", "전기장판", "온수매트", "히터", "온풍기", "난로"),
            {
                K_PRODUCT_GROUP: "난방가전",
                K_SCENARIO: "방 전체 난방보다 필요한 공간만 따뜻하게 쓰고 싶을 때",
                K_TARGET: "겨울철 난방비가 부담되는 자취생, 침실 사용자, 사무실 사용자",
                K_PLACE: "침실, 책상 아래, 원룸, 사무실",
                K_SEASON: "겨울, 환절기",
                K_PAIN: "전기요금, 안전장치, 크기, 보관성을 같이 보지 않으면 만족도가 낮아질 수 있음",
                K_POINT1: "공간별 보조 난방으로 접근하기 쉬움",
                K_POINT2: "가격대와 사용 시간이 구매 판단에 직접 연결됨",
                K_POINT3: "안전장치와 온도 조절 여부를 비교하기 좋음",
                K_CAUTION: "장시간 사용 시 안전장치, 소비전력, KC 인증 여부를 확인해야 함",
            },
        ),
        (
            ("공기청정기", "필터", "알콜스왑", "제습제", "습기"),
            {
                K_PRODUCT_GROUP: "공기/위생 관리용품",
                K_SCENARIO: "집 안 공기질, 냄새, 습기, 위생 관리가 신경 쓰일 때",
                K_TARGET: "원룸, 아이 있는 집, 반려동물 가정, 실내 생활 시간이 긴 사람",
                K_PLACE: "원룸, 침실, 거실, 옷장, 욕실",
                K_SEASON: "사계절, 장마철",
                K_PAIN: "효과 범위와 교체 주기를 모르고 사면 체감이 약할 수 있음",
                K_POINT1: "반복 구매나 관리 수요로 이어지기 쉬움",
                K_POINT2: "공간 크기와 사용 주기를 기준으로 판단하기 좋음",
                K_POINT3: "가격 부담이 비교적 낮아 상세 정보 확인 장벽이 낮음",
                K_CAUTION: "사용 면적, 교체 주기, 소모품 비용을 같이 확인해야 함",
            },
        ),
        (
            ("\uC804\uAE30\uD3EC\uD2B8", "\uC8FC\uC804\uC790"),
            {
                K_PRODUCT_GROUP: "\uC804\uAE30\uD3EC\uD2B8",
...
def scrape_products(
    ...
    print(
        f"[crawl] 로켓배송 > {TARGET_CATEGORY_NAME} > {TARGET_SUBCATEGORY_NAME} > 현재 노출 순서 > {MIN_PRODUCT_PRICE:,}원 이상"
    )
    try:
            ...
            try:
                ...
                collected.append(
                    build_row(
                        ...
                        collection_keyword=TARGET_SUBCATEGORY_NAME,
                    )
                )
...
def save_products(rows: Iterable[ProductRow], output_path: Path) -> None:
    ...
    used_json_path = output_path.parent / "자동발행상태기록파일" / "coupang_used_products.json"
    if used_json_path.exists():
        try:
            with used_json_path.open("r", encoding="utf-8") as f:
                used_data = json.load(f)
            for key, val in used_data.items():
                key_link = str(key or "").strip()
                link = str(val.get("product_link", "") or "").strip()
                name = str(val.get("product_name", "") or "").strip()
                if key_link:
                    used_urls.add(key_link)
                    existing_product_keys.update(build_product_identity_keys(product_url=key_link))
                if link:
                    used_urls.add(link)
                    existing_product_keys.update(build_product_identity_keys(product_url=link))
                if name:
                    used_names.add(name)
                    existing_product_keys.update(build_product_identity_keys(product_name=name))
        except Exception as exc:
            print(f"[warn] failed to load used products before save: {exc}")
